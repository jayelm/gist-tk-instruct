...
def set_argument_value(arguments, name, value):
    ...
    assert not (
        isinstance(arguments[idx + 1], str) and arguments[idx + 1].startswith("-")
    )  # make sure the next argument is the value
...
if experiment_group == "num_of_tasks":
    ...
    for train_task_num in train_task_nums:
        ...
        set_argument_value(
            d["tasks"][0]["command"], "--master_port", random.randint(25000, 35000)
        )
        set_argument_value(
            d["tasks"][0]["command"],
            "--data_dir",
            f"/data/cross_category/train_{train_task_num}",
        )
...
if experiment_group == "model":
    ...
    for model_name in model_names:
        ...
        if "small" in model_name:
            set_argument_value(
                d["tasks"][0]["command"], "--per_device_train_batch_size", 16
            )
            set_argument_value(
                d["tasks"][0]["command"], "--per_device_eval_batch_size", 32
            )
            set_argument_value(
                d["tasks"][0]["command"], "--gradient_accumulation_steps", 1
            )
            d["tasks"][0]["resources"]["gpuCount"] = 1
        elif "base" in model_name:
            set_argument_value(
                d["tasks"][0]["command"], "--per_device_train_batch_size", 8
            )
            set_argument_value(
                d["tasks"][0]["command"], "--per_device_eval_batch_size", 16
            )
            set_argument_value(
                d["tasks"][0]["command"], "--gradient_accumulation_steps", 1
            )
            ...
            set_argument_value(
                d["tasks"][0]["command"], "--per_device_train_batch_size", 4
            )
            set_argument_value(
                d["tasks"][0]["command"], "--per_device_eval_batch_size", 8
            )
            set_argument_value(
                d["tasks"][0]["command"], "--gradient_accumulation_steps", 1
            )
            ...
            set_argument_value(
                d["tasks"][0]["command"], "--per_device_train_batch_size", 2
            )
            set_argument_value(
                d["tasks"][0]["command"], "--per_device_eval_batch_size", 8
            )
            ...
            set_argument_value(
                d["tasks"][0]["command"], "--per_device_train_batch_size", 1
            )
            set_argument_value(
                d["tasks"][0]["command"], "--per_device_eval_batch_size", 8
            )
            ...
            d["tasks"][0]["resources"]["gpuCount"] = 8
            d["tasks"][0]["command"].remove(
                "--bf16"
            )  # stage 3 is currently 4x slower with bf16
            # set_argument_value(d['tasks'][0]['command'], "--max_source_length", 1024)
            set_argument_value(d["tasks"][0]["command"], "--generation_max_length", 10)
            set_argument_value(
                d["tasks"][0]["command"], "--deepspeed", "ds_configs/stage3.config"
            )
...
if experiment_group == "hyper_tuning":
    ...
    for lr in learning_rates:
        for acc_step in acc_steps:
            ...
            set_argument_value(
                d["tasks"][0]["command"], "--gradient_accumulation_steps", acc_step
            )
            set_argument_value(d["tasks"][0]["command"], "--run_name", name)
            ...
            cmd = "beaker experiment create {} --workspace ai2/yizhong_default".format(
                fn
            )
            subprocess.Popen(cmd, shell=True)
...
if experiment_group == "encoding":
    for encoding_name, encoding in encodings.items():
        ...
        set_argument_value(
            d["tasks"][0]["command"], "--add_task_name", encoding["add_task_name"]
        )
        set_argument_value(
            d["tasks"][0]["command"],
            "--add_task_definition",
        ...
        set_argument_value(
            d["tasks"][0]["command"], "--num_pos_examples", encoding["num_pos_examples"]
        )
        set_argument_value(
            d["tasks"][0]["command"], "--num_neg_examples", encoding["num_neg_examples"]
        )
        set_argument_value(
            d["tasks"][0]["command"], "--add_explanation", encoding["add_explanation"]
        )
        if "tk_instruct" in encoding:
            set_argument_value(
                d["tasks"][0]["command"], "--tk_instruct", encoding["tk_instruct"]
            )
...
if experiment_group == "test_sets":
    ...
    for set_idx in range(10):
        ...
        set_argument_value(
            d["tasks"][0]["command"],
            "--data_dir",
            f"/data/cross_category/set_{set_idx}",
        )
...
if experiment_group == "splits":
    ...
    for split_name in ["default", "no_synthetic", "supervised"]:
        ...
        d["tasks"][0]["name"] = name
        set_argument_value(
            d["tasks"][0]["command"], "--data_dir", f"/data/cross_category/{split_name}"
        )
    ...
    for model_name in model_names:
        for encoding_name, encoding in encodings.items():
            ...
            set_argument_value(
                d["tasks"][0]["command"],
                "--num_pos_examples",
                encoding["num_pos_examples"],
            )
            set_argument_value(
                d["tasks"][0]["command"],
                "--num_neg_examples",
                encoding["num_neg_examples"],
            )
            set_argument_value(
                d["tasks"][0]["command"],
                "--add_explanation",
                encoding["add_explanation"],
            )
            ...
            set_argument_value(
                d["tasks"][0]["command"], "--model_name_or_path", model_name
            )
            d["tasks"][0]["resources"]["gpuCount"] = 1
            if "small" in model_names:
                ...
                set_argument_value(
                    d["tasks"][0]["command"], "--per_device_eval_batch_size", 4
                )
            elif (
                "11b" in model_name
                or "11B" in model_name
                or "-xxl" in model_name
                or model_name == "bigscience/T0"
            ):
                set_argument_value(
                    d["tasks"][0]["command"], "--per_device_eval_batch_size", 2
                )
                d["tasks"][0]["resources"]["gpuCount"] = 8
                set_argument_value(
                    d["tasks"][0]["command"], "--deepspeed", "ds_configs/stage3.config"
    ...
    for checkpoint_name, beaker_dataset_id, checkpoint_step in checkpoints:
        for encoding_name, encoding in encodings.items():
            ...
            d["tasks"][0]["datasets"].append(
                {"mountPath": "/models/", "source": {"beaker": beaker_dataset_id}}
            )
            set_argument_value(
                d["tasks"][0]["command"],
                "--model_name_or_path",
                "/models/" + (checkpoint_step if checkpoint_step else ""),
            )
...
if experiment_group == "supervised":
    for encoding_name, encoding in encodings.items():
        ...
        set_argument_value(
            d["tasks"][0]["command"], "--data_dir", f"/data/supervised/multilingual"
        )
        set_argument_value(
            d["tasks"][0]["command"], "--max_num_instances_per_task", 1000
        )
        set_argument_value(d["tasks"][0]["command"], "--run_name", name)
...
if experiment_group == "multilingual":
    ...
    for model_name in model_names:
        ...
        if "small" in model_name:
            ...
            set_argument_value(
                d["tasks"][0]["command"], "--gradient_accumulation_steps", 2
            )
            d["tasks"][0]["resources"]["gpuCount"] = 8
        elif "11b" in model_name or "-xxl" in model_name:
            set_argument_value(
